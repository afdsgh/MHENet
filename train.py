...
logging.basicConfig(filename=f'{save_path}{model_name}.log',
                    format='[%(asctime)s-%(filename)s-%(levelname)s:%(message)s]', level=logging.INFO, filemode='a',
                    datefmt='%Y-%m-%d %I:%M:%S %p')
logging.info(f"{model_name}-Train")
model = MHENet()
...
print(f"Number of GPUs used: {torch.cuda.device_count()}")  # 应输出实际使用的GPU数量
# train function
def train(train_loader, model, optimizer, epoch, save_path):
    ...
    try:
        ...
        if epoch % 5 == 0 and epoch >= 40:
            torch.save(model.state_dict(), f'{save_path}{model_name}_epoch_{epoch}.pth')
    ...
    with torch.no_grad():
        ...
        if epoch == 1:
            ...
            if mae < best_mae:
                best_mae = mae
                best_epoch = epoch
                torch.save(model.state_dict(), f'{save_path}{model_name}_epoch_best.pth')
