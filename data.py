...
def randomOcclusion(img, label, depth, max_occlusion=0.3):
    w = img.size[0]
    h = img.size[1]
    occlusion_w = random.randint(0, int(w * max_occlusion))
    occlusion_h = random.randint(0, int(h * max_occlusion))
    x = random.randint(0, w - occlusion_w)
    y = random.randint(0, h - occlusion_h)

    # 创建遮挡区域（灰色矩形）
    occlusion = Image.new('RGB', (occlusion_w, occlusion_h), (128, 128, 128))
    img.paste(occlusion, (x, y))

    # 对标签和深度图使用黑色遮挡
    label.paste(0, (x, y, x + occlusion_w, y + occlusion_h))
    depth.paste(0, (x, y, x + occlusion_w, y + occlusion_h))
    return img, label, depth
...
class SalObjDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        gt = randomPeper(gt)
        # image, gt, depth = randomOcclusion(image, gt, depth)
